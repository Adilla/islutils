            ...
            switch(k) {
                case Gemm :
                    //if (findGemm(ctx, scop, reads, writes) == true)
                        return findGemm(ctx, scop, reads, writes);
                    break;
                // case Transpose : {
...
findGemm(isl::ctx ctx,
    ...
    if (_isGemm == true) {
        ...
        auto test = reads.get_map_list();
        //auto val = test.get_map(0).get_space().get_tuple_name(isl::dim::out);
        ...
        if (accessdom.is_subset(scheddom)) {
            ...
            isGemm = findGemmTree(ctx, subnode, _node);
            //std::cout << isGemm << std::endl;
	        
            
            
            //subnode.dump();
            //subnode.ancestor(1).dump();
            // If ancestor is root then do not cut. 
            // int countgeneration = 0;
            // isl::schedule_node track = subnode;
            // while(track.has_parent()) {
            //     countgeneration += 1;
            //     track = track.ancestor(countgeneration);
            // }
            // std::cout << countgeneration << std::endl;

            // auto newroot = track;
            // track.dump();
            
            
            //auto newnode = domain(newroot.domain_get_domain(),
              //                  extension(isl::union_map(ctx, "{[]->gemm[]:"))).build();
...
//             auto dependences = computeAllDependences(scop);
//             subnode = mergeIfTilable(subnode, dependences);
//             isTransposeGemm = findGemmTree(ctx, subnode, _node);
//         }
//     }
...
//             auto dependences = computeAllDependences(scop);
//             subnode = mergeIfTilable(subnode, dependences);
//             isBatchGemm = findGemmTree(ctx, subnode, _node);
//         }
//     }
//     return isBatchGemm;
//                                 }
