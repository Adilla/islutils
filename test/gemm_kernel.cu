...
__global__ void kernel0(int *A, int *B, int *D, int alpha)
{
...

    {
      ...
      for (int c2 = 0; c2 <= 1023; c2 += 32) {
        ...
        for (int c3 = 0; c3 <= 31; c3 += 1) {
          private_D[0][0] += (((alpha) * shared_A[t0][c3]) * shared_B[c3][t1]);
          private_D[0][1] += (((alpha) * shared_A[t0][c3]) * shared_B[c3][t1 + 16]);
        }
        __syncthreads();
      }
