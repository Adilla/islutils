	...
	struct gpu_prog *prog)
{
	...
	/* Blasmatch: declare handle */
	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p, "cublasHandle_t handle;");
	p = isl_printer_end_line(p);
	...
	__isl_take isl_printer *p, struct gpu_prog *prog)
{
	...
	/* Blasmatch: allocate handle */
	isl_printer_start_line(p);
	isl_printer_print_str(p, "cudaCheckReturn(cublasCreate(&handle));");
	...
	struct gpu_prog *prog)
{
	...
	/* Blasmatch: clean up the allocation for handle */
	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p, "cudaCheckReturn(cublasDestroy(handle));");
	p = isl_printer_end_line(p);
...
/* Blasmatch: do the same as copy_array_to_device but with cublas function */
static __isl_give isl_printer *cublas_copy_array_to_device(
	__isl_take isl_printer *p, struct gpu_array_info *array)
{

	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p, "cudaCheckReturn(cublasSetMatrix(");
	p = cublas_gpu_array_info_print_size(p, array);
	p = isl_printer_print_str(p, ", ");
	p = isl_printer_print_str(p, array->name);
	p = isl_printer_print_str(p, ", ");
	p = cublas_gpu_array_info_print_bound(p, array, 1);
	p = isl_printer_print_str(p, ", dev_");
	p = isl_printer_print_str(p, array->name);
	p = isl_printer_print_str(p, ", ");
	p = cublas_gpu_array_info_print_bound(p, array, 1);
...
/* Blasmatch: do the same as copy_array_from_device but with cublas function */
static __isl_give isl_printer *cublas_copy_array_from_device(
	__isl_take isl_printer *p, struct gpu_array_info *array)
{
	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p, "cudaCheckReturn(cudaGetMatrix(");
	p = cublas_gpu_array_info_print_size(p, array);
	p = isl_printer_print_str(p, ", dev_");
	p = isl_printer_print_str(p, array->name);
	p = isl_printer_print_str(p, ", ");
...
/* Blasmatch: similarly to print_kernel_arguments, we are going to 
 * define how to print cublas kernel calls. 
 * In this case, parameters vary depending on the BLAS
 */
static __isl_give isl_printer *cublas_print_kernel_arguments(
	__isl_take isl_printer *p, struct gpu_prog *prog, 
	...
	__isl_keep isl_ast_node *node, struct gpu_prog *prog)
{
	...
	if (!prefixcmp(name, "to_device"))
		return cublas_copy_array_to_device(p, array);
	else 
		return cublas_copy_array_from_device(p, array);
}
