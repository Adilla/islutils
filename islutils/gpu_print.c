...
/* Blasmatch: print sizes in the context of cuBlasSetMatrix and cublasGetMatrix */
__isl_give isl_printer *cublas_gpu_array_info_print_size(
	__isl_take isl_printer *prn, struct gpu_array_info *array) 
{
	int i;
...
/* Blasmatch: return an array bound for a given position */
__isl_give isl_printer *cublas_gpu_array_info_print_bound(
	__isl_take isl_printer *prn, struct gpu_array_info *array,
	int pos)
{
	isl_ast_expr *bound;
	...
	//prn = isl_printer_print_str(prn, "(");
	bound = isl_ast_expr_get_op_arg(array->bound_expr, 1);
	prn = isl_printer_print_ast_expr(prn, bound);
	isl_ast_expr_free(bound);
	//prn = isl_printer_print_str(prn, ")");
